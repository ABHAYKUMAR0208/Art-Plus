      ...
      <div className="container px-5 py-24 mx-auto flex flex-wrap">
              ...
              {aboutLinks.length > 0 ? (
                aboutLinks.map((link) => (
                  <li key={link.label}>
                    <Link
                      to={link.url}
                      className="text-gray-600 hover:text-gray-900 transition-transform transform hover:scale-105 inline-block duration-300"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="w-full lg:w-1/2 px-4 flex flex-wrap lg:flex-nowrap">
          ...
          <div className="w-full lg:w-1/2 px-4">
            ...
            <p className="text-gray-600 transition-transform transform hover:scale-105 inline-block duration-300">
              {contactInfo.address}
            </p>
            {/* Phone Numbers - Displayed on Separate Lines */}
            {contactInfo.phone.map((phone, index) => (
              <span
                key={index}
                className="block text-gray-600 transition-transform transform hover:scale-105 duration-300"
              >
                {phone}
              </span>
            ))}
            ...
            <div className="flex mt-4">
              {contactInfo.socialLinks.map((social, index) => (
                <a
                  key={index}
                  href={social.url}
                  className="mr-4 transform hover:scale-125 transition-transform duration-300"
          ...
          <div className="w-full lg:w-1/2 px-4 mt-8 lg:mt-0">
            ...
            <div className="flex flex-wrap items-center group">
              <input
                ...
                className="w-full p-2 rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none text-gray-700 text-sm transform hover:scale-105 transition-transform duration-300"
              />
              <button className="p-2  mt-2 bg-indigo-500 text-white rounded hover:bg-indigo-600 transform hover:scale-105 transition-transform duration-300">
                Subscribe
              </button>
